...
//! substitute: a feature may declare a tag that differs from its stem, and
//! going by file name would then invent resources that nothing owns while
//! missing the ones that exist.
...
mod tests {
    ...
    fn parses_the_first_non_special_tag() {
        assert_eq!(
            parse("@serial\n@tunnel_basic\nFeature: x\n"),
            Some("tunnel_basic".to_string())
        );
        assert_eq!(
            parse("@serial @allow.skipped @tunnel_mtu\nFeature: x\n"),
            Some("tunnel_mtu".to_string())
        );
