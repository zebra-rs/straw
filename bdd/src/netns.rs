...
/// repo — `straw`, `strawc`, `test_client` — resolve to this worktree's
/// build instead of anything installed under `/usr/bin`. System tools (`ip`,
/// `bridge`, `ping`, `curl`, `python3`) resolve exactly as before.
///
...
/// unique within that namespace; feature-file step arguments reference
/// this name, so callers typically pick the bare
...
/// Spawn a process in a namespace with stdout and stderr appended to
/// `log_path`.
///
/// The daemons write their tracing output to stderr rather than to a
/// `--log-file`, so the harness has to capture it: several assertions read
/// the log to prove what the daemon decided (the negotiated tunnel MTU, a
/// refused route). The file is opened for append so a restart within one
/// scenario keeps the earlier lines; `World::mark_log_start` records the
/// offset so assertions only see the current run's output.
pub async fn spawn_in_netns_logged(
    netns: &str,
    env: &[(&str, &str)],
    cmd: &str,
    args: &[&str],
    ...
    c.arg(cmd);
    c.args(args);
    c.stdout(Stdio::from(log)).stderr(Stdio::from(errlog));
    c.spawn()
        .with_context(|| format!("Failed to spawn {} in netns {}", cmd, netns))
}
...
///
/// We delete via `sudo rm` because the daemon is launched with `sudo ip
...
/// SIGTERM the PID recorded in `path`, wait up to `grace` for it to exit,
/// then SIGKILL whatever is left, and remove the file.
///
/// The daemons undo their kernel changes on SIGTERM — `strawc` removes the
/// addresses and routes it installed, `straw` its NAT rules — and several
/// scenarios assert exactly that, so a plain SIGKILL would test nothing.
/// Missing file, dead process, or unparseable contents are not errors.
pub async fn terminate_pidfile(path: &Path, grace: std::time::Duration) -> Result<()> {
    if let Some(pid) = read_pid(path).await {
        let pid_str = pid.to_string();
        let _ = Command::new("sudo")
            .args(["kill", "-TERM", &pid_str])
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .status()
            .await;
        let deadline = tokio::time::Instant::now() + grace;
        while pidfile_alive(path).await && tokio::time::Instant::now() < deadline {
            tokio::time::sleep(std::time::Duration::from_millis(100)).await;
        }
    }
    kill_pidfile(path).await
}
