...
| P1 | `udp_bind/` at relay **(codec+alloc+socket done: `udp_bind/{context,alloc,socket}.rs`; connect-udp handler + dispatch next)**; token v2 **(done: `p2p/token.rs`)**; peer identity + SPKI pinning **(done: `p2p/identity.rs`)**; inner QUIC through relay (Phase A+B); `strawcat/1` ALPN pipes | P0 | ~2–3 weeks |
