...
| P1 | `udp_bind/` at relay **(done: `udp_bind/{context,alloc,socket,handler}.rs` + `:protocol` dispatch + demux routing; relays UDP end to end)**; token v2 **(done: `p2p/token.rs`)**; peer identity + SPKI pinning **(done: `p2p/identity.rs`)**; inner QUIC through relay (Phase A+B); `strawcat/1` ALPN pipes | P0 | ~2–3 weeks |
