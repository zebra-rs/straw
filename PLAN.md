...
3. **TUN portability** — resolved. `forwarding/tun/` and `iface/` both split per platform, so macOS runs the client (utun, ifconfig, route). The proxy stays on Linux by deployment choice, so pf/NAT is out of scope rather than pending.
